...
```{r csardl-fit}
# CS-ARDL: Levels, adjustment, and long-run parameters
...
**Interpretation**: CS-ARDL returns levels ARDL coefficients, adjustment, and long-run ratios. It does not return the complete transformed ECM short-run parameterization. A long-run interpretation requires appropriate stability and economic assumptions; inspect the stored unit diagnostics and component sample counts.
...
set.seed(1234)
cd_dcce <- cd_test(fit_dcce, type = "CDw", na.action = "drop.incomplete.times", seed = 42)
print(cd_dcce)

# Test CS-ARDL residuals for CSD
set.seed(1234)
cd_csardl <- cd_test(fit_csardl, type = "all", na.action = "drop.incomplete.times", seed = 42)
print(cd_csardl)
```
