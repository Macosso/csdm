...
- $\beta_{i}$ is a ((k )) vector of unit-specific slopes
- $x_{it}$ is a ((k )) vector of explanatory variables
...
[`csdm_csa()`](reference/csdm_csa.md), for example
...
[`csdm_lr()`](reference/csdm_lr.md) and the cross-sectional averages and
their lags are controlled through [`csdm_csa()`](reference/csdm_csa.md).
...
where the dynamic structure is controlled through
[`csdm_lr()`](reference/csdm_lr.md) and the cross-sectional averages are
supplied through [`csdm_csa()`](reference/csdm_csa.md).
...
- [`csdm_csa()`](reference/csdm_csa.md) defines which variables enter as
...
The CDw statistic uses unit-level random sign flips to form a wild-type
version of the CD test:

$$CD_{w} = \sqrt{\frac{2}{N(N - 1)}}\sum\limits_{i = 1}^{N - 1}\sum\limits_{j = i + 1}^{N}w_{i}w_{j}\,{\widehat{\rho}}_{ij}\sqrt{T},$$

where $\left( w_{1},\ldots,w_{N} \right)$ are independent random weights
with $w_{i} \in \{ - 1,1\}$ applied at the unit level. This statistic
can be used in randomization-based or simulation-based inference
procedures.
...
where $\left( w_{1}^{( + )},\ldots,w_{N}^{( + )} \right)$ are again
independent random weights with $w_{i}^{( + )} \in \{ - 1,1\}$
(typically a separate draw from that used for $CD_{w}$).
...
high-dimensional cross-section tests. Econometrica, 83(4), 1497–1541.
...
panels with multifactor error structure. Econometrica, 74(4), 967–1012.
...
cross-sectional dependence in panel models. Econometric Reviews, 41(6),
649–677.
